...
1. Экономия вычислительных ресурсов:
   ...
   - Двухуровневый нейросетевой каскад (рекомендуется для продакшена): скоростной Tier-1 триаж TypeSafe Jev отсекает до 80-90% заведомо чистых сообщений за 150-400 мс, направляя к ресурсоемким генеративным моделям (DeepSeek, Groq) только подозрительные инциденты.
...
3. Tier-1 triage (TypeSafe Jev, рекомендуется для продакшена, `JEV_ENABLED=true`):
   - Высокоскоростная дискриминативная классификация System One без накладных расходов на генерацию чат-токенов: `noul` (вероятность нарушения) + `choice` по 9 категориям.
   - Время ответа: 150-400 мс против 2.5-4 секунд у генеративных моделей, что критично для поддержания живого диалога в активных чатах.
   - Fast-Pass: чистые сообщения (`prob < JEV_FAST_PASS_THRESHOLD`, по умолчанию 0.03) безопасно пропускаются без вызова дорогой LLM, сокращая расходы на токены на 80-90%.
   - Принцип Defense-Only: Jev используется только для быстрого допуска чистого трафика; любые карательные санкции (варн, мут, бан) требуют валидации и русскоязычного обоснования от большой LLM.
   - Эскалация: подозрительные сообщения направляются в LLM с хинтом определенной Jev категории; критические угрозы (`illegal_contraband`, `phishing`) и скрытые ссылки эскалируются безусловно.
   - Отказоустойчивость: таймауты, лимиты 429/529 или сетевые сбои автоматически переключают поток на прямую отправку в DeepSeek; circuit breaker изолирует неисправный ключ без падения бота.
   - Метрики: детальная запись каждого вызова в `logs/jev_triage.jsonl` для калибровки порогов и аналитики.
...
Предупреждения автоматически аннулируются по истечении установленного срока (по умолчанию 7 дней, `Chat.warn_expiration_days`).
