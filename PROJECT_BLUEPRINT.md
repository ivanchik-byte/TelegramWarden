...
3. Tier-1 triage (TypeSafe Jev, опционально, `JEV_ENABLED`):
    ...
    - Чистые (`prob < JEV_FAST_PASS_THRESHOLD`, по умолчанию 0.03) пропускаются без LLM; запись метрик каждого triage: в `logs/jev_triage.jsonl` для калибровки порога.
    ...
    - Любой сбой Jev (таймаут, 429/529, сеть) дает прозрачный fallback на LLM; неверный ключ отключает tier через circuit breaker.
