...
Система защиты и администрирования Telegram-сообществ с многоуровневым конвейером: пре-фильтрация без расхода токенов, локальные ONNX-модели компьютерного зрения (Yahoo Open-NSFW), классификация через LLM (DeepSeek / Groq) и встроенная панель управления Telegram Mini App.

> Статус: версия v1.1.0-stable. Кодовая база покрыта набором из 109 автоматизированных тестов. Вопросы по развертыванию или найденным багам можно отправлять в личные сообщения или открывать в issues.
...
- **Анализ текста без лишних вызовов API**:
  ...
  - Скоринг риска по репутации пользователя в чате: сообщения проверенных участников пропускаются сразу, без обращения к нейросетям.
...
| `JEV_ENABLED`| Нет | Tier-1 triage через TypeSafe Jev (без ключа отключен) | `false` |
...
| `JEV_FAST_PASS_THRESHOLD`| Нет | Порог fast-pass для чистых сообщений | `0.03` |
...
docker compose up -d
```
4. Настройте регулярный бэкап тома PostgreSQL (`warden_postgres_data`).
